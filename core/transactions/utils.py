...
def update_products_from_invoice(invoice_data_processed, request):
    for item in invoice_data_processed['items']:
        ...
        if Item.objects.filter(
                                  name=item['invoice_product'],
                                  hsn=item['invoice_hsn'],
                                  unit=item['invoice_unit'],
                                  gst_percentage=item['invoice_gst_percentage']).exists():
            product = Item.objects.get(
                                         name=item['invoice_product'],
                                          hsn=item['invoice_hsn'],
                                          unit=item['invoice_unit'],
                                          gst_percentage=item['invoice_gst_percentage'])
        else:
            new_product = True
            product = Item(
                              name=item['invoice_product'],
                              ...
                              unit=item['invoice_unit'],
                              gst_percentage=item['invoice_gst_percentage'])
        product.rate_with_gst = item['invoice_rate_with_gst']
        product.save()
