...
function customer_result_click() {
    ...
    $('#customer-gst-input').val(customer_data_json['gst']);
}
...
function initialize_fuse_customers () {
    // fetch customer data
    $.getJSON( "http://127.0.0.1:8000/transactions/customersjson", function( data ) {
...
function product_result_to_domstr(result) {
    ...
    "<div>"+ result['name'] + "</div>" +
    "<div>"+ result['hsn'] + " | " + result['unit'] + " | " + result['gst_percentage'] +
    "</div>";
     return domstr;
}
...
function product_result_click() {
    ...
    selected_item_input.parent().parent().find('input[name=invoice-rate-with-gst]').val(product_data_json['rate_with_gst']);
...
function initialize_fuse_products () {
    // fetch customer data
    $.getJSON( 'http://127.0.0.1:8000/transactions/productsjson', function( data ) {
